...
use DateTime;
	...
	class Node {
		...
		function __construct($name, $value, $ttl = 0) {
		...
		function destroy($timestamp) {
			...
			// Do not delete any cache key that has ttl = -1;
			if ($this->_ttl == -1) {
				return false;
			}
			// If the ttl is set, delete accordingly
			else if ($this->_ttl > 0) {
			...
			// Otherwise, delete by the expiry schedule
